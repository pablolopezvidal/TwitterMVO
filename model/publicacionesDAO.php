...
function selectPublications($pdo) {
    try {
        ...
        foreach ($statement->fetchAll() as $p) {
            $objectP = new Publications($p["id"], $p["userId"], $p["text"],$p["createDate"], $p["username"]);# la solucion que propongo es en vez de crear un obgeto publicacion como tal creo un ibject que sea un array que si tenga en nombre entre sus partes y envio eso
